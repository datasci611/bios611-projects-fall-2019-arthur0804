...
for (i in 2000:2018){
    ...
    # extract the record for last year
    df_clients_last_year <- filter(df_clients, df_clients$year == (i-1))
    
    # get the unique client numbers of this year
    count_last_year <- unique(df_clients_last_year$ClientFileNumber)
    
    # append count of new clients to the dataframe
    new_clients_count = length(setdiff(count_this_year, count_last_year))
